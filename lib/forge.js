...
Forge.prototype._write = function(done) {
    ...
    async.forEachSeries(app.items.value(), function(item, next) {
        item.write(app, next);
        app.itemsByPath[item.target] = item;
    }, done);
};

Forge.prototype._cleanup = function(done) {
    var app = this;
    new Tree(this.output).traverse(function(source, stat, done) {
        var route = source.substring(this.root.length);
        var item = app.itemsByPath[route];
        if (!item) {
            console.warn('[%s] %s', route, 'deleting');
            fs.unlink(source, done);
        } else {
            console.warn('[%s] %s', route, item.status);
            done();
        }
    }, done);
};
