    ...
    load: function(done) {
        ...
        fs.readFile(item.source, 'utf8', function(err, data) {
            ...
            if (match) {
                ...
                ['route', 'template'].forEach(function(prop) {
                    if (item.metadata[prop]) {
                        item[prop] = item.metadata[prop];
                        delete item.metadata[prop];
                    }
                });
        ...
        mkdirp(path.dirname(target), function(err) {
            ...
            if (output !== null) {
                ...
                fs.writeFile(target, output, function(err) {
                    ...
                    else console.warn('[%s] Written.', target);
                    done();
                });
            } else if (item.source) {
                ...
                util.pump(from, to, function(err) {
                    if (err) item.reportError(err);
                    else console.warn('[%s] Copied.', target);
                    done();
                });
            } else {
...
//     metadata: {
//         // Loaded from file
//         // if file contains 'route' property, migrate it to top
//     },
//     content: '...'
// }